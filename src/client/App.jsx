...
export default function App() {
  ...
  return <main>
    ...
    <section className="panel campaign-panel"><div className="panel-title"><div><p>LIVE SCAN 2 CAMPAIGN</p><h2>Broader wallet coverage · {campaign.allTimeTargetApiCalls ?? 1000} genuine Nansen calls all-time</h2></div><span>{campaign.completedRuns} completed cycles</span></div>
      <div className="campaign-layout"><div><div className="campaign-count"><strong>{campaign.allTimeApiCalls ?? campaign.cumulativeRealNansenApiCalls ?? 0}</strong><span>/ {campaign.allTimeTargetApiCalls ?? 1000} GENUINE NANSEN CALLS · ALL TIME</span></div><div className="progress"><i style={{ width: `${Math.min(100, ((campaign.allTimeApiCalls ?? campaign.cumulativeRealNansenApiCalls ?? 0) / (campaign.allTimeTargetApiCalls || 1000)) * 100)}%` }} /></div><p className="campaign-split">{campaign.campaignCallsActual ?? campaign.campaignCallsSent ?? 0} from Live Scan 2 campaign cycles ({campaign.campaignCallsSent ?? 0} recorded in the cap ledger{campaign.campaignCallsUnrecorded ? `, plus ${campaign.campaignCallsUnrecorded} from early runs whose totals the legacy ledger never stored` : ""}) + {campaign.otherApiCalls ?? 0} from plain Run Live Scan = {campaign.allTimeApiCalls ?? 0} genuine Nansen requests. Every real request counts toward the 1,000-call buildathon minimum, including retries; cache hits and requests rejected before transmission never count.</p><div className="campaign-count live-api-count"><strong>{status.totalCalls ?? 0}</strong><span>/ {status.runCallLimit ?? status.apiCallLimit ?? 909} LIVE API CALLS THIS RUN · HARD CAP</span><small>{status.spendableCredits != null ? `${status.creditsReserved ?? 0}/${status.spendableCredits} credits spent this run · 5-credit reserve` : ""}</small></div><p>Each Live Scan 2 cycle collects BUY/SELL wallet evidence, flow context, and holder depth across every qualifying token until the run budget is spent or distinct research runs out, then tests genuine cross-token overlap. A 15-minute cooldown prevents duplicate runs. One run may send up to {status.runCallLimit ?? status.apiCallLimit ?? 909} real Nansen requests, retries included, and the campaign total never exceeds {status.apiCallLimit ?? 909}. When the Nansen balance is known the cycle is credit-aware: it spends at most {status.spendableCredits ?? "the balance"} credits (a 5-credit safety reserve is kept) and fills that spend with 1-credit research before ever scheduling a 5-credit holder call, so scarce credits buy the maximum number of genuine, distinct requests.</p></div><div className="campaign-action"><button className="secondary" disabled={status.running || !campaign.canRun || !Number.isSafeInteger(Number(creditLimit)) || Number(creditLimit) < 10 || Number(creditLimit) > 200} onClick={runCampaign}>{campaign.callsRemaining === 0 ? "TARGET COMPLETE" : "RUN LIVE SCAN 2"}</button><small>{campaign.callsRemaining ?? 909} genuine calls remaining{!campaign.canRun && campaign.callsRemaining > 0 && campaign.nextEligibleAt ? ` · next cycle ${new Date(campaign.nextEligibleAt).toLocaleTimeString()}` : ""}</small></div></div>
